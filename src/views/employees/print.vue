  ...
  <div id="myPrint" class="dashboard-container">
    <div class="app-container">
      <el-card>
        ...
        <el-button v-print="{ id: 'printbox' }" type="primary" size="small"
          >打印</el-button
        >
        <div v-if="type === 'personal'" id="printbox">
          ...
          <table cellspacing="0" width="100%" class="tableList">
            ...
            <tr>
              <th>员工照片</th>
              <td><img :src="formData.staffPhoto" class="imgcss" /></td>
              <th>生日</th>
...
export default {
  ...
  methods: {
    async getPersonalDetail() {
      const { data: userInfo } = await getStaffInfo(this.userId) // 获取个人基本信息(顶部)
      const { data: detail } = await reqGetPersonalDetail(this.userId) // 获取个人基本信息(底部)
      this.formData = { ...detail, ...userInfo }
    },
    async getJobDetail() {
      const { data: userInfo } = await getStaffInfo(this.userId)
